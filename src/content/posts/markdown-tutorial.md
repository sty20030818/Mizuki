...
一个展示如何编写 Markdown 文件的示例。本文档整合了核心语法和扩展语法（GFM）。
...
使用任意数量的**等号 (=)** 作为 `<h1>`，使用**短横线 (-)** 作为 `<h2>` 来"下划线"。
...
可选地，你可以"关闭" atx 样式的标题。关闭的井号**不需要匹配**用于打开标题的井号数量。
...
Markdown 使用电子邮件风格的 **>** 字符来引用。如果你对文本进行硬换行并在每行前面加上 >，效果最好。
...
以下部分的围栏代码块和语法高亮是扩展功能，你可以使用另一种方式编写代码块。
...
在围栏块中，添加一个可选的语言标识符，我们将通过语法高亮运行它（[支持的语言](https://github.com/github/linguist/blob/master/lib/linguist/languages.yml)）。
...
在一行中单独放置**三个或更多短横线 (-)、星号 (\*) 或下划线 (\_)**。你可以在短横线或星号之间使用空格。
...
外部的**竖线 (|)** 和对齐是可选的。分隔表头时每个单元格至少需要 **3 个分隔符**。
...
Markdown 将**星号 (\*)** 和**下划线 (\_)** 视为强调的指示符。**单个分隔符**将是 `<em>`；**双分隔符**将是 `<strong>`。
...
要在代码范围内包含字面反引号字符，可以使用**多个反引号**作为开始和结束分隔符：
...
围绕代码范围的反引号分隔符可以包含空格 —— 开始后一个，结束前一个。这允许你在代码范围的开头或结尾放置字面反引号字符：
...
Markdown 使用类似于链接语法的图片语法，允许两种样式：内联和引用。
...
- 后跟一组括号，包含图片的 URL 或路径，以及用双引号或单引号括起来的可选标题属性。
...
Markdown 支持为 URL 和电子邮件地址创建"自动"链接的快捷样式：只需用尖括号包围 URL 或电子邮件地址。
...
Markdown 允许你使用反斜杠转义来生成在 Markdown 格式语法中具有特殊含义的字面字符。
...
对于 Markdown 语法未涵盖的任何标记，你只需使用 HTML 本身。无需在前面加上前缀或分隔符来表示你正在从 Markdown 切换到 HTML；你只需使用标签。
